...
function visualMarkup(event, index) {
  if (event.image) {
    ...
    const captionText = isOrigin ? "《尚书》古本" : event.caption;
    ...
    return `<figure class="artifact ${isOrigin ? "paper" : ""} ${isRelic ? "relic-roundel" : ""} ${imageClass}">
      <img src="${event.image}" alt="${captionText}" loading="lazy">
...
function installVisibleImageHover() {
  ...
  function layoutCaption(artifact, visibleRect) {
    ...
    if (artifact.matches(".image-origin")) {
      ...
      const top = Math.max(
        pad,
        Math.min(
          visibleRect.top + visibleRect.height * 0.8 - capRect.height / 2,
