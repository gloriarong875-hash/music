(() => {
  ...
  var streamObjectType = {
    ...
    FeatureCollection: function(object, stream) {
      var features2 = object.features, i = -1, n = features2.length;
  ...
  function rejoin_default(segments, compareIntersection2, startInside, interpolate3, stream) {
  ...
  function circle_default(radius) {
    ...
    return clip_default(visible, clipLine, interpolate3, smallRadius ? [0, -radius] : [-pi, radius - pi]);
  }
  ...
  function path_default(projection3, context3) {
    ...
    return path.projection(projection3).digits(digits).context(context3);
  }
  ...
  var chronicle = [
    {
      ...
      track: "\u300A\u653E\u9A74\u300B",
      audio: "../../public/audio/sheng/fang-lv.mp3",
      mapLabel: "\u6CB3\u5317\u7701 \xB7 \u5180\u4E2D\u5E73\u539F",
      provinceAdcode: 13e4,
      center: [115.3, 38.3],
      zoom: 1.82
    },
    ...
    },
    {
      ...
      track: "\u300A\u51E4\u51F0\u5C55\u7FC5\u300B",
      audio: "../../public/audio/sheng/feng-huang-zhan-chi.mp3",
      mapLabel: "\u5C71\u897F\u7701 \xB7 \u592A\u539F / \u5FFB\u5DDE",
      provinceAdcode: 14e4,
      center: [112.55, 38.4],
      zoom: 1.94
    },
  ...
  var eraPositions = [[47, 10], [31, 29], [51, 48], [34, 68], [49, 88]];
  var mapSize = { width: 1e3, height: 620 };
  var reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  var elements = {
    workspace: document.querySelector(".workspace"),
    eraList: document.querySelector("#eraList"),
    progress: document.querySelector(".rail-progress"),
    mapViewport: document.querySelector("#mapViewport"),
    baseLayer: document.querySelector("#baseMapLayer"),
    highlightLayer: document.querySelector("#highlightLayer"),
    routeGuide: document.querySelector("#routeGuide"),
    route: document.querySelector("#routePath"),
    routeArrow: document.querySelector("#routeArrow"),
    point: document.querySelector("#mapPoint"),
    halo: document.querySelector("#mapHalo"),
    label: document.querySelector("#placeLabel"),
    loading: document.querySelector("#mapLoading"),
    card: document.querySelector("#archiveCard"),
    cardTitle: document.querySelector("#cardTitle"),
    cardLocation: document.querySelector("#cardLocation"),
    ...
    audioCurrent: document.querySelector("#audioCurrent"),
  ...
  var activeIndex = 0;
  var currentView = "chronicle";
  var features = [];
  var projection2;
  var pathGenerator;
  var cameraTimeline;
  var cameraPoint = [500, 310];
  var cameraZoom = 1;
  var trackAudio = new Audio();
  ...
  var pageOrder = ["sound", "painting", "chronicle"];
  ...
  function renderMap(data) {
    ...
    projection2 = mercator_default().fitExtent([[62, 48], [938, 572]], collection);
    pathGenerator = path_default(projection2);
    ...
    if (!reduceMotion) gsapWithCSS.from("#baseMapLayer .map-area", { opacity: 0, duration: 1.1, stagger: 2e-3, ease: "power3.out" });
  }
  function renderHighlights(item) {
    ...
    if (!reduceMotion) gsapWithCSS.fromTo("#highlightLayer .selected-area", { opacity: 0, scale: 0.985, transformOrigin: "center" }, { opacity: 1, scale: 1, duration: 0.75, stagger: 0.035, ease: "power3.out" });
  }
  function projectedPoint(item) {
    return projection2 ? projection2(item.center) : [500, 310];
  }
  function makeRoute(from, to) {
    const midX = (from[0] + to[0]) / 2;
    const lift = Math.max(34, Math.abs(to[0] - from[0]) * 0.2);
  ...
  function updateScreenRoute(guideLength, cameraCenter, zoom, progress = 1) {
    ...
    elements.routeArrow.style.opacity = progress > 0.015 ? "1" : "0";
  }
  ...
  function setMapAtTarget(item) {
    ...
    gsapWithCSS.set([elements.point, elements.halo], { attr: { cx: point[0], cy: point[1] } });
    elements.label.textContent = item.mapLabel;
    positionLabel(point, zoom, point);
    gsapWithCSS.set(elements.label, { autoAlpha: 1 });
  }
  function animateCameraTracking(from, item) {
    const to = projectedPoint(item);
    const targetZoom = item.zoom || 1.5;
    const travelZoom = Math.min(1.04, cameraZoom * 0.72, targetZoom * 0.68);
    const guideLength = prepareRoute(from, to);
    const tracker = { progress: 0 };
    cameraTimeline?.kill();
    gsapWithCSS.killTweensOf([elements.mapViewport, elements.route, elements.routeArrow, elements.point, elements.halo, elements.label, elements.card]);
    updateScreenRoute(guideLength, from, travelZoom, 0);
    gsapWithCSS.set([elements.route, elements.routeArrow], { opacity: 0 });
    gsapWithCSS.set(elements.label, { autoAlpha: 0 });
    cameraTimeline = gsapWithCSS.timeline({
    ...
    cameraTimeline.to(elements.card, { autoAlpha: 0, x: 44, scale: 0.96, duration: 0.28 }, 0).to(elements.highlightLayer, { opacity: 0.12, duration: 0.32 }, 0).call(() => fillChronicleCard(item), [], 0.3).to(elements.mapViewport, { attr: { transform: cameraTransform(from, travelZoom) }, duration: 0.58 }, 0).set(elements.route, { opacity: 0.94 }, 0.42).to(tracker, {
      progress: 1,
      duration: 1.75,
      ease: "power1.inOut",
      onUpdate: () => {
    ...
    }, 0.58).to([elements.route, elements.routeArrow], { opacity: 0, duration: 0.32, ease: "power2.out" }, 2.26).to(elements.mapViewport, { attr: { transform: cameraTransform(to, targetZoom) }, duration: 0.78, ease: "expo.out" }, 2.35).to([elements.point, elements.halo], { attr: { cx: to[0], cy: to[1] }, duration: 0.55, ease: "expo.out" }, 2.35).call(() => {
      renderHighlights(item);
      gsapWithCSS.set(elements.highlightLayer, { opacity: 1 });
      elements.label.textContent = item.mapLabel;
      positionLabel(to, targetZoom, to);
    }, [], 2.6).to(elements.label, { autoAlpha: 1, duration: 0.32 }, 2.72).to(elements.card, { autoAlpha: 1, x: 0, scale: 1, duration: 0.62, ease: "expo.out" }, 2.84);
  }
  ...
  function selectEra(index, animate = true) {
    ...
    const from = projection2 ? cameraPoint : [500, 310];
    ...
    if (projection2) {
      if (animate && !reduceMotion) {
        ...
        renderHighlights(item);
        setMapAtTarget(item);
        const routeStart = activeIndex === 0 ? projection2([111.5, 35.2]) : projectedPoint(chronicle[activeIndex - 1]);
        const guideLength = prepareRoute(routeStart, projectedPoint(item));
        updateScreenRoute(guideLength, projectedPoint(item), Math.min(1.04, (item.zoom || 1.5) * 0.68), 1);
        gsapWithCSS.set([elements.route, elements.routeArrow], { opacity: 0 });
        gsapWithCSS.set(elements.card, { autoAlpha: 1, x: 0, scale: 1 });
      }
    } else {
      fillChronicleCard(item);
    }
  ...
  function setView(view) {
    ...
    if (view === "object") {
      elements.stageKicker.textContent = "SHENG \xB7 MATERIAL STUDY";
      elements.stageTitle.textContent = "\u530F\u6597\u3001\u7C27\u7247\u4E0E\u97F3\u7BA1";
      elements.cardTitle.textContent = "\u7B19\u7684\u6784\u9020";
      elements.cardLocation.textContent = "\u7C27\u7247 \xB7 \u97F3\u7BA1 \xB7 \u7B19\u6597 \xB7 \u5439\u53E3";
      ...
      elements.stageKicker.textContent = "SHENG \xB7 SOUND PHENOMENON";
      ...
      elements.stageKicker.textContent = "SHENG \xB7 CHRONICLE";
  ...
  var railPageSwitcher = document.querySelector("#railPageSwitcher");
  ...
  elements.audioSeek.addEventListener("pointerdown", () => {
    isSeeking = true;
  });
  ...
  elements.audioSeek.addEventListener("change", () => {
    isSeeking = false;
  });
  elements.audioSeek.addEventListener("pointerup", () => {
    isSeeking = false;
  });
  ...
  var requestedEra = Number(new URLSearchParams(window.location.search).get("era"));
  activeIndex = Number.isInteger(requestedEra) ? Math.max(0, Math.min(4, requestedEra)) : 0;
  selectEra(activeIndex, false);
  try {
    renderMap(china_default);
  } catch (error) {
